...
def lambda_handler(event, context):
    ...
    if namespace == "Alexa.Authorization":
        ...
        # TODO Do backend get based on endpointId
        ...
        if name == "AdjustBrightness":
            # TODO limit 0..100
            btn = btn + int(event.get("directive").get("payload").get("brightnessDelta"))
        elif name == "SetBrightness":
            btn = event.get("directive").get("payload").get("brightness")

        # TODO Send the request and wait for the operation success - async?
